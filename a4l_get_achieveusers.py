from api42lib import IntraAPIClient
import os, sys, json, datetime, math
import my_common as my

var = {
    # "cursus_name": "42cursus",
    # "campus_name": "Tokyo",
    # "test_user": ""
}

def get_achieveusers(achieve_id = None):
    ic = IntraAPIClient(config_path="./config.yml")

    # params = {
    #     "filter[name]": var['campus_name'],
    # }
    # campus_id = ic.get("campus", params=params).json()[0].get("id")

    params = {
        "sort": "created_at"
    }
    if achieve_id != None:
        params["filter[achievement_id]"] = achieve_id
    achieveusers = ic.pages_threaded(f"achievements_users/", params=params)

    return achieveusers

def wrapper(args):
    if len(args) > 1:
        achieve_id = int(args[1])
    else:
        achieve_id = None
    return get_achieveusers(achieve_id)

if __name__ == "__main__":
    start_at = datetime.datetime.now()

    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    data_dir = BASE_DIR + "/data/achieveusers"
    my.mkdir(data_dir, True)
    date = my.get_datetime()
    filepath = "achieveusers_" + date + ".json"
    filepath = data_dir + "/" + filepath

    ret = wrapper(sys.argv)
    my.save_json(ret, filepath)

    finish_at = datetime.datetime.now()
    print(f"Elapsed time: {finish_at - start_at}")
